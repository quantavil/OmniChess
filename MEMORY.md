...
- **Brand Assets Revamp, Image Audit & Usage Page Removal:** Replaced deleted ACAS logo brand images with premium monochromatic dark-theme assets. Ran a codebase audit to identify and remove all unused assets. Removed 'extgui' (external server GUI) and 'support' (Monero donation modal) throughout the codebase. Deleted the entire `usage` folder along with its related assets (`example5.png`, `chart.png`, `render-example.png`) and redirected all documentation links to the FAQ page. Programmatically rendered a pixel-perfect chessboard mockup in an opening position. Renamed image assets to follow a descriptive schema (`social-preview`, `showcase-overlay`, `showcase-settings`, `showcase-full`) and integrated them as centered headers and tables in `README.md` and `userscript/README.md`.
